    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
          <div>
            ...
            <h2 className="text-sm font-bold text-slate-800 mt-0.5">
              #{d._id?.slice(-10).toUpperCase()}
            </h2>
          </div>
          <button
            onClick={onClose}
            className="h-8 w-8 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 transition"
            aria-label="Close"
          >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* Associate */}
          <SectionLabel>Assigned Associate</SectionLabel>
          <div className="mx-4 bg-white rounded-xl border border-slate-200 overflow-hidden">
            {d.associateId ? (
              <>
          ...
          {/* Service */}
          <SectionLabel>Service / Product</SectionLabel>
          <div className="mx-4 bg-white rounded-xl border border-slate-200 overflow-hidden">
            {d.services?.map((sv, i) => (
...
export default function AdminDeliveries() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await deliveryAPI.getAllDeliveries(params);
      ...
      if (from) {
        ...
        data = data.filter(d => {
          const ts = statusFilter === 'delivered' && d.deliveredAt
            ? new Date(d.deliveredAt) : new Date(d.createdAt);
          return ts >= fromDate;
        });
      }
      if (to) {
    ...
    <>
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold text-slate-800">District Deliveries</h2>
            <p className="text-xs text-slate-400 mt-0.5">Click any row to view full details</p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          ...
          <div className="px-4 py-3 border-b border-slate-100">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by member, associate, block, service, or ID…"
                value={search}
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-slate-400">
                {search ? 'No deliveries match your search.' : 'No deliveries found for the selected period.'}
              </p>
            </div>
          ) : (
            <>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3.5 px-4">
                            ...
                            {d.memberId?.memberId && (
                              <p className="text-[10px] font-mono text-emerald-600">{d.memberId.memberId}</p>
                            )}
                            {d.memberId?.phone && (
                              <p className="text-[10px] text-slate-400">{d.memberId.phone}</p>
                            )}
                          </td>
                          ...
                          <td className="py-3.5 px-4">
                            {d.associateId ? (
                              <>
                                ...
                                {d.associateId.employeeId && (
                                  <p className="text-[10px] font-mono text-violet-600">{d.associateId.employeeId}</p>
                                )}
                                {d.associateId.phone && (
                                  <p className="text-[10px] text-slate-400">{d.associateId.phone}</p>
                                )}
                              </>
                            ) : (
                    ...
                    >
                      ...
                      {d.associateId && (
                        <p className="text-[10px] text-violet-600 mb-1">
                          👤 {d.associateId.name}
                          {d.associateId.employeeId && <span className="font-mono ml-1">({d.associateId.employeeId})</span>}
                        </p>
                      )}
                      ...
                      <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-500">
                        ...
                        {d.services?.map((sv, i) => (
                          <span key={i} className="text-slate-700">{sv.serviceId?.name || '—'}</span>
                        ))}
                      ...
                      <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1 text-[10px] text-slate-400">
                        ...
                        {!d.deliveredAt && d.estimatedDeliveryDate && (
                          <span className="text-amber-500">🗓 Est. {fmt(d.estimatedDeliveryDate)}</span>
                        )}
                      </div>
                      {d.failReason && (
                        <p className="mt-1 text-[10px] text-red-500">❌ {d.failReason}</p>
                      )}
                    </button>
                  );
                })}
